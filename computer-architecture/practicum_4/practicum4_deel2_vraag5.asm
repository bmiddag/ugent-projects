...
; .............
	...
	push	Taak1
	call	creeertaak
	add	esp, 12      ; beter voor de stapel, de argumenten van deze functie zijn niet meer nodig dus ze mogen overschreven worden

	push	1000
	push	stapel2
	push	Taak2
	call	creeertaak
	add	esp, 12      ; zie hierboven
...
; oproepen als creeertaak(adres, stapel, wachttijd)
	...
	mov    ebx, [esp+8]
	mov    eax, [esp+4]
	add    ebx, STAPELGROOTTE
	mov   edx, esp
	mov   esp, ebx
	push  200h	; flags register
	push  cs	; cs
	push  eax	; adres
	pushad
	mov   ebx, [Huidige_Taak]
.creeertaak_zoeklus: 	; zoeken naar een vrije plaats in takenlijst
	add	ebx, 8
	cmp	ebx, takenlijst + (MAX_TAKEN * 8)
	jl	.creertaak_nog_niet_aan_het_einde
	lea	ebx, [takenlijst]
.creertaak_nog_niet_aan_het_einde:
	cmp	dword [ebx], 0
	jne	.creeertaak_zoeklus
	mov [ebx], esp
	mov [ebx + 4], ecx
	mov esp, edx
	ret
...
geentaak_string db "GEEN TAAK", 0
...
.nog_niet_aan_het_einde:
        cmp	dword [ebx],0
        je	.taakzoeklus_deel1
        cmp     dword [ebx+4], ecx
        jg      .taakzoeklus_deel1
...
.geentaken:
	push	11
	push	35
	push	geentaak_string
	call	printstring
	add	esp, 12      ; beter voor de stapel, deze argumenten zijn niet meer nodig dus ze mogen overschreven worden
	mov	edx, 0         ; edx wordt veranderd door printstring dus we moeten edx terug een goede waarde geven
	jmp	.taakzoeklus_deel2
